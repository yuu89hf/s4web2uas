<x-app-layout>
    ...
    <div class="py-8" x-data="{
        ...
        loading: false,
        searchQuery: '', // State baru untuk menampung teks pencarian
        
        async refreshFeed() {
            if (this.loading) return;
            this.loading = true;
            this.searchQuery = ''; // Reset pencarian saat memuat data baru
        ...
        // Fungsi Helper untuk menyaring artikel berdasarkan judul atau isi teks secara real-time
        get filteredArticles() {
            if (this.searchQuery.trim() === '') {
                return this.articles;
            }
            return $data.articles.filter(article => {
                return article.title.toLowerCase().includes($data.searchQuery.toLowerCase()) || 
                       article.extract.toLowerCase().includes($data.searchQuery.toLowerCase());
            });
        }
    }">
